...
fn generate_test(out: &mut Output) {
    ...
    for i in 0..n {
        out.print_iter((0..m).map(|_| r.gen() % 3 + 1));
        out.print_line("");
    }
}
...
fn stupid(input: &mut Input, out: &mut Output) {
    ...
    for i in 0..n {
        let mut row = vec![];
        for j in 0..m {
            row.push(input.read_long());
        }
        matrix.push(row);
    }

    fn permute_rows(matrix: &mut Vec<Vec<i64>>, start: usize, result: &mut Vec<Vec<Vec<i64>>>) {
        if start == matrix.len() {
        ...
        for i in start..matrix.len() {
            matrix.swap(start, i);
            permute_rows(matrix, start + 1, result);
            matrix.swap(start, i);
        }
    }

    let mut permutations = Vec::new();
    permute_rows(&mut matrix, 0, &mut permutations);
    ...
    for matrix in permutations {
        ...
        for i in 1..n {
            for j in 0..m {
                cur_ans.minim((matrix[i][j] - matrix[i - 1][j]).abs());
            }
        }
        for j in 1..m {
            cur_ans.minim((matrix[n - 1][j - 1] - matrix[0][j]).abs());
        }
        ans.maxim(cur_ans);
    }
    out.print_line(ans);
...
pub(crate) fn run_tests() -> bool {
    ...
    let mut paths = std::fs::read_dir("./f_elongated_matrix/tests/")
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
    ...
    loop {
        ...
        match res {
            ...
            Err(err) => {
                ...
                println!("Error: {}", err);
                // TODO: ask user if we want to save this test to tests dir. Keep in mind multitest setup
                panic!();
            }
        }
    }
