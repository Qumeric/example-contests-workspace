...
fn generate_test(out: &mut Output) {
    ...
    let amount = r.gen() % LEN + 1;
    out.print_line(&amount);

    let v = (0..amount).map(|_| r.gen() % LEN + 1).collect_vec();
    out.print_line(v);
}
...
fn stupid(input: &mut Input, out: &mut Output) {
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            elems.sort();
            elems.dedup();
            ans += elems.len() as f64;
        }
    }

    out.print_line((ans / (n * n) as f64).to_string());
}
...
pub(crate) fn run_tests() -> bool {
    ...
    let mut paths = std::fs::read_dir("./f_random_query/tests/")
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
