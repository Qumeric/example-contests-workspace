...
/// This trait defines the method to find strongly connected components in a graph.
///
/// Strongly connected components are subsets of vertices within a directed graph
/// where every vertex is reachable from every other vertex in the same subset.
///
/// # Returns
///
/// A tuple containing:
/// - A vector of indices representing the component each vertex belongs to.
/// - A graph where each vertex represents a strongly connected component and
///   each edge represents connections between components.
